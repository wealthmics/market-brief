...
def find_newsletter():
    ...
    if not files:
        raise SystemExit(
            f"No .html file found in the {INPUT_DIR}/ folder. "
...
def find_snippet():
    """
    Find the newest market snapshot HTML.
    ...
    if not files:
        raise SystemExit(
            "No market snapshot HTML found. "
            "Did the snapshot script run successfully?"
        )
...
def main():
    ...
    # IST, because the GitHub workflow sets TZ=Asia/Kolkata
    ...
    if not block.search(html):
        raise SystemExit(
            ...
            f"{os.path.basename(newsletter_path)}. "
            "Those comment markers must stay."
        )
    ...
    date_div = re.compile(
        r"(color:rgba\(255,255,255,0\.90\);"
        r"margin-top:4px;"
        r"letter-spacing:0\.5px;\">\s*)"
        r"([^<]+?)(\s*</div>)",
        re.DOTALL,
    )
    ...
    stable = os.path.join(
        OUTPUT_DIR,
        "Global_Market_Brief_latest.html"
    )

    dated = os.path.join(
        OUTPUT_DIR,
        f"Global_Market_Brief_{today:%Y-%m-%d}.html"
    )
    ...
    # ── 6) COPY EXCEL SNAPSHOT ───────────────────────────────────────────────

    xlsx = sorted(
        glob.glob("MICS_Market_Snapshot_*.xlsx"),
        key=os.path.getmtime
    )
    ...
    print(
        f"Input used   : {newsletter_path}"
    )

    print(
        f"Snippet used : {snippet_path}"
    )

    print(
        f"Issue set to : {issue:03d} "
        f"({n_issue} place)"
    )

    print(
        f"Date set to  : {masthead_date(today)} "
        f"({n_date} masthead, {n_title} title)"
    )
